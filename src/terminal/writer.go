...
func FormatTitle(title string) string {
	...
	for _, char := range title {
		EscapeSequence, shouldEscape := formats.EscapeSequences[char]
		if shouldEscape {
			s.WriteString(EscapeSequence)
			continue
		}
...
func write(s rune) {
	...
	if !Interactive && !Plain {
		EscapeSequence, shouldEscape := formats.EscapeSequences[s]
		if shouldEscape {
			builder.WriteString(EscapeSequence)
			return
		}
	}
